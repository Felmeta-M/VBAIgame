...
class DialogueSystem:
    ...
    async def initialize_realtime(self):
        ...
        try:
            async with self.client.beta.realtime.connect(model="gpt-4o-realtime-preview-2024-10-01") as conn:
                ...
                # Configure session for better audio handling
    ...
    def start_audio_recording(self):
        ...
        try:
            # Check if we have a valid Realtime connection
            if not self.realtime_conn:
                print("[DialogueSystem] Warning: No Realtime API connection. Audio recording may not work.")
            
            # Check audio devices
            try:
                devices = sd.query_devices()
                input_device = sd.query_devices(kind='input')
                print(f"[DialogueSystem] Using input device: {input_device['name']}")
            except Exception as e:
                print(f"[DialogueSystem] Warning: Could not query audio devices: {e}")
            ...
            print(f"[DialogueSystem] Error starting audio recording: {e}")
            import traceback
            traceback.print_exc()
    ...
    def stop_audio_recording(self):
        """Stop recording audio and send to Realtime API"""
        ...
        try:
            ...
            self.is_recording = False
            print("[DialogueSystem] Audio recording stopped")
            
            # Send audio to Realtime API
            asyncio.run_coroutine_threadsafe(self.send_audio_to_realtime(), self.loop)
            
        except Exception as e:
            print(f"[DialogueSystem] Error stopping audio recording: {e}")

    async def process_audio_input(self):
        """Process audio input and send to Realtime API"""
        if not self.realtime_conn or not self.is_recording:
            return
            
        try:
            while self.is_recording and self.realtime_conn:
                if self.audio_stream and self.audio_stream.read_available > 0:
                    data, _ = self.audio_stream.read(self.audio_stream.read_available)
                    if len(data) > 0:
            ...
            print(f"[DialogueSystem] Error processing audio: {e}")

    async def send_audio_to_realtime(self):
        """Send accumulated audio to Realtime API and request response"""
        if not self.realtime_conn:
            return
        ...
        try:
            ...
            print(f"[DialogueSystem] Error sending audio to Realtime: {e}")

    def check_audio_system_status(self):
        """Check the status of the audio system and provide diagnostics"""
        print("\n=== Audio System Status ===")
    ...
    def render(self):
        ...
        if self.active:
            ...
            # Quit instruction
            quit_text_surface = self.font.render("Press Shift+Q to exit, F5 to record voice, F6 for audio status", True, (255, 255, 255))
    ...
    def handle_input(self, event):
        ...
        if event.type == pygame.KEYDOWN:
            ...
            if event.key == pygame.K_F6:
                self.check_audio_system_status()
